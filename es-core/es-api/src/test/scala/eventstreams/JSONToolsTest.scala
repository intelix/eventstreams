...
import _root_.core.sysevents.support.EventAssertions
...
class JSONToolsTest extends FlatSpec with Matchers with EventAssertions with BeforeAndAfterEach with StrictLogging  {
  ...
  "toPath" should "work for p1" in {
    JSONTools.toPath("p1") should be (__ \ 'p1)
  }
  it should "work for p1.p2" in {
    JSONTools.toPath("p1.p2") should be (__ \ 'p1 \ 'p2)
  }
  it should "work for p1.p2.p3" in {
    JSONTools.toPath("p1.p2.p3") should be (__ \ 'p1 \ 'p2 \ 'p3)
  }
  it should "work for p1/p2.p3" in {
    JSONTools.toPath("p1/p2.p3") should be (__ \ 'p1 \ 'p2 \ 'p3)
  }
  ...
  it should "work for p1(1)" in {
    JSONTools.toPath("p1(1)") should be ((__ \ 'p1)(1))
  }
  it should "work for p1(1).p2" in {
    JSONTools.toPath("p1(1).p2") should be ((__ \ 'p1)(1) \ 'p2)
  }
  it should "work for p1(2).p2(1).p3" in {
    JSONTools.toPath("p1(2).p2(1).p3") should be (((__ \ 'p1)(2) \ 'p2)(1) \ 'p3)
  }
  it should "work for p1(1)/p2(0).p3(3)" in {
    JSONTools.toPath("p1(1)/p2(0).p3(3)") should be ((((__ \ 'p1)(1) \ 'p2)(0) \ 'p3)(3))
  }
  it should "work for p1/p2/p3(1)" in {
    JSONTools.toPath("p1/p2/p3(3)") should be ((__ \ 'p1 \ 'p2 \ 'p3)(3))
  }
  it should "work for p1 (1)/p2( 0).p3 ( 3 ) " in {
    JSONTools.toPath("p1 (1)/p2( 0).p3 ( 3 ) ") should be ((((__ \ 'p1)(1) \ 'p2)(0) \ 'p3)(3))
  }
  ...
  s"macroReplacement for input $testInput" should "resolve mnb into mnb" in {
    JSONTools.macroReplacement(testFrame,"mnb") should be ("mnb")
  }
  it should "resolve ${f1} into abc" in {
    JSONTools.macroReplacement(testFrame,"${f1}") should be ("abc")
  }
  it should "resolve ${f1} into abc if we use overloaded method" in {
    JSONTools.macroReplacement(testInput, Map[String,String](), "${f1}") should be ("abc")
  }
  ...
  it should "resolve ${fx} into abc1 if we provide value in the context" in {
    JSONTools.macroReplacement(testInput, Map[String,String]("fx" -> "abc1"), "${fx}") should be ("abc1")
  }

  it should "resolve ${f1}_x into abc_x" in {
    JSONTools.macroReplacement(testFrame,"${f1}_x") should be ("abc_x")
  }
  it should "resolve x_${f1} into x_abc" in {
    JSONTools.macroReplacement(testFrame,"x_${f1}") should be ("x_abc")
  }
  it should "resolve x_${fz} into x_" in {
    JSONTools.macroReplacement(testFrame,"x_${fz}") should be ("x_")
  }
  it should "resolve x_${f2} into x_true" in {
    JSONTools.macroReplacement(testFrame,"x_${f2}") should be ("x_true")
  }
  it should "resolve x_${f2}${f1} into x_trueabc" in {
    JSONTools.macroReplacement(testFrame,"x_${f2}${f1}") should be ("x_trueabc")
  }
  it should "resolve x_${f2.f1}${f1.f2} into x_" in {
    JSONTools.macroReplacement(testFrame,"x_${f2.f1}${f1.f2}") should be ("x_")
  }
  it should "resolve x_${f3} into x_123.1" in {
    JSONTools.macroReplacement(testFrame,"x_${f3}") should be ("x_123.1")
  }
  it should "resolve x_${f4} into x_\"a1\",\"a2\"" in {
    JSONTools.macroReplacement(testFrame,"x_${f4}") should be ("x_\"a1\",\"a2\"")
  }
  it should "resolve x_${f5.z1} into x_xyz" in {
    JSONTools.macroReplacement(testFrame,"x_${f5.z1}") should be ("x_xyz")
  }
  it should "resolve x_${f5/z1} into x_xyz" in {
    JSONTools.macroReplacement(testFrame,"x_${f5/z1}") should be ("x_xyz")
  }
  ...
  it should "resolve now:${now:yyyy-MMM} into "+expectedNow in {
    JSONTools.macroReplacement(testFrame,"now:${now:yyyy-MMM}") should be (s"now:$expectedNow")
  }
  it should "resolve evt:${eventts:yyyy-MMM-dd_HH:mm:ss} into "+expectedEvt in {
    JSONTools.macroReplacement(testFrame,"evt:${eventts:yyyy-MMM-dd_HH:mm:ss}") should be (s"evt:$expectedEvt")
  }
  ...
  s"macroReplacement for input $testInputNoDateTs" should "resolve evt:${eventts:yyyy-MMM-dd_HH} into "+expectedEvtBasedOnNow in {
    JSONTools.macroReplacement(testInputNoDateTs, Map[String,String](),"evt:${eventts:yyyy-MMM-dd_HH}") should be (s"evt:$expectedEvtBasedOnNow")
  }

  s"setValue(s,JsString(qwe)...)for input $testInputNoDateTs " should "set string into path" in {
    JSONTools.setValue("s","qwe",'path, testInputNoDateTs) ~> "path" should be (Some("qwe"))
  }
  it should "set string into f1" in {
    JSONTools.setValue("s","qwe",'f1, testInputNoDateTs) ~> "f1" should be (Some("qwe"))
  }
  it should "set string into f2" in {
    JSONTools.setValue("s","qwe",'f2, testInputNoDateTs) ~> "f2" should be (Some("qwe"))
  }
  it should "set string into f3" in {
    JSONTools.setValue("s","qwe",'f3, testInputNoDateTs) ~> "f3" should be (Some("qwe"))
  }
  it should "set string into f4" in {
    JSONTools.setValue("s","qwe",'f4, testInputNoDateTs) ~> "f4" should be (Some("qwe"))
  }

  s"setValue(n,JsString(qwe)...)for input $testInputNoDateTs " should "set string or num into path" in {
    JSONTools.setValue("n","qwe",'path, testInputNoDateTs) ~> "path" should be (Some("0"))
    JSONTools.setValue("n","qwe",'path, testInputNoDateTs) +> "path" should be (Some(0))
  }

  s"setValue(n,JsString(12)...)for input $testInputNoDateTs " should "set number into path" in {
    JSONTools.setValue("n","12",'path, testInputNoDateTs) +> "path" should be (Some(12))
  }
  it should "set number into f1" in {
    JSONTools.setValue("n","12",'f1, testInputNoDateTs) +> "f1" should be (Some(12))
  }
  it should "set number into f2" in {
    JSONTools.setValue("n","12",'f2, testInputNoDateTs) +> "f2" should be (Some(12))
  }
  it should "set number into f3" in {
    JSONTools.setValue("n","12",'f3, testInputNoDateTs) +> "f3" should be (Some(12))
  }
  it should "set number into f4" in {
    JSONTools.setValue("n","12",'f4, testInputNoDateTs) +> "f4" should be (Some(12))
  }
  ...
  s"setValue(s,12...)for input $testInputNoDateTs " should "set string into path" in {
    JSONTools.setValue("s","12",'path, testInputNoDateTs) ~> "path" should be (Some("12"))
  }
  it should "set string into f1" in {
    JSONTools.setValue("s","12",'f1, testInputNoDateTs) ~> "f1" should be (Some("12"))
  }
  it should "set string into f2" in {
    JSONTools.setValue("s","12",'f2, testInputNoDateTs) ~> "f2" should be (Some("12"))
  }
  it should "set string into f3" in {
    JSONTools.setValue("s","12",'f3, testInputNoDateTs) ~> "f3" should be (Some("12"))
  }
  it should "set string into f4" in {
    JSONTools.setValue("s","12",'f4, testInputNoDateTs) ~> "f4" should be (Some("12"))
  }

  s"setValue(b,JsString(true)...)for input $testInputNoDateTs " should "set bool into path" in {
    JSONTools.setValue("b","true",'path, testInputNoDateTs) ?> "path" should be (Some(true))
  }
  it should "set bool into f1" in {
    JSONTools.setValue("b","true",'f1, testInputNoDateTs) ?> "f1" should be (Some(true))
  }
  it should "set bool into f2" in {
    JSONTools.setValue("b","true",'f2, testInputNoDateTs) ?> "f2" should be (Some(true))
  }
  it should "set bool into f3" in {
    JSONTools.setValue("b","true",'f3, testInputNoDateTs) ?> "f3" should be (Some(true))
  }
  it should "set bool into f4" in {
    JSONTools.setValue("b","true",'f4, testInputNoDateTs) ?> "f4" should be (Some(true))
  }

  s"setValue(as,JsString(qwe)...)for input $testInputNoDateTs " should "set string arr into path" in {
    JSONTools.setValue("as","qwe",'path, testInputNoDateTs) ##> "path" should be (Some(List(EventDataValueString("qwe"))))
  }
  it should "set string into f1" in {
    JSONTools.setValue("as","qwe",'f1, testInputNoDateTs) ##> "f1" should be (Some(List(EventDataValueString("abc"),EventDataValueString("qwe"))))
  }
  it should "set string into f2" in {
    JSONTools.setValue("as","qwe",'f2, testInputNoDateTs) ##> "f2" should be (Some(List(EventDataValueBoolean(true),EventDataValueString("qwe"))))
  }
  it should "set string into f3" in {
    JSONTools.setValue("as","qwe",'f3, testInputNoDateTs) ##> "f3" should be (Some(List(EventDataValueNumber(123.1),EventDataValueString("qwe"))))
  }
  it should "set string into f4" in {
    JSONTools.setValue("as","qwe",'f4, testInputNoDateTs) ##> "f4" should be (Some(List(EventDataValueString("a1"),EventDataValueString("a2"),EventDataValueString("qwe"))))
  }

  "UUIDTools.generateShortUUID" should "generate a random short uuid" in {
    UUIDTools.generateShortUUID should not be UUIDTools.generateShortUUID
  }
  ...
  it should "be a string" in {
    UUIDTools.generateShortUUID shouldBe a [String]
  }
  it should "be longer than 10 symbols" in {
    UUIDTools.generateShortUUID.length should be > 10
  }
