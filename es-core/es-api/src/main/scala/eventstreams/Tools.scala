...
object Tools {
  ...
  implicit def optionsHelper[A](o: Option[A]): ExtraOptionOps[A] = new ExtraOptionOps(o)


  final class ExtraOptionOps[A](self: scala.Option[A]) {
    def orFail(s: String): \/[Fail, A] = self match {
      case Some(v) => \/-(v)
      case _ => Fail(s)
    }
  }
