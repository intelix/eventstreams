...
package eventstreams.sources.jmx
...
class JMXEventsource extends BuilderFromConfig[Props] with StrictLogging {
   override def configId: String = "jmx"

   def build(props: JsValue, maybeState: Option[JsValue], id: Option[String] = None): \/[Fail, Props] = {
     logger.debug(s"Building JMXEventsource from $props")
