...
import eventstreams.instructions.{DateInstruction, DateInstructionConstants, SimpleInstructionBuilder}
...
class DateInstructionTest extends TestHelpers with DateInstructionConstants {
  ...
  it should "properly parse a valid input" in new WithMinimalFromTsConfig {
    expectEvent(EventFrame("sourceField" -> System.currentTimeMillis()))(DateParsed)
  }
  it should "not fail with an invalid input" in new WithMinimalFromTsConfig {
    expectEvent(EventFrame("abc" -> System.currentTimeMillis()))(DateParsed)
  }
  it should "not fail with an invalid input (string)" in new WithMinimalFromTsConfig {
    expectEvent(EventFrame("sourceField" -> "123"))(DateParsed)
  }
  ...
  it should "properly parse a valid input" in new WithMinimalFromStringConfig {
    expectEvent(EventFrame("sourceField" -> "2014-Jan-10"))(DateParsed)
  }
  it should "not parse an invalid input" in new WithMinimalFromStringConfig {
    expectEvent(EventFrame("abc" -> System.currentTimeMillis()))(UnableToParseDate)
  }
  it should "not parse an invalid input (long)" in new WithMinimalFromStringConfig {
    expectEvent(EventFrame("sourceField" -> System.currentTimeMillis()))(UnableToParseDate)
  }

  it should "populate default ts field" in new WithMinimalFromStringConfig {
    expectOne(EventFrame("sourceField" -> "2014-Jan-10")) { result =>
  ...
  s"DateInstruction WithMinimalFromStringConfig (syd source, with time)" should "populate all components" in new WithMinimalFromStringWithTimeConfig {
    expectOne(EventFrame("sourceField" -> "2014-Jan-10 22:11:12.123")) { result =>
  ...
  it should "not process invalid sources (pattern mismatch)" in new WithMinimalFromStringWithTimeConfig {
    expectOne(EventFrame("sourceField" -> "2014-Jan-10 22:11:12")) { result =>
  ...
  it should "not process invalid sources (invalid minute value)" in new WithMinimalFromStringWithTimeConfig {
    expectOne(EventFrame("sourceField" -> "2014-Jan-10 22:61:12")) { result =>
      (result ~> default_targetFmtField ) should be(None)
    }
  }
